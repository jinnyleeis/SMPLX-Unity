...
{
    public bool issetblendshapeactive;
    ...
    public void SetBodyPose(BodyPose pose)
    {
        if (pose == BodyPose.T)
        {
            ResetBodyPose(); // 기본 포즈로. 
        }
        else if (pose == BodyPose.A)
        {
            ResetBodyPose(); // 기본 포즈로. - 이유? 
    ...
    public void UpdatePoseCorrectives()
    {
        ...
        for (int i=1; i<_bodyJointNames.Length; i++)
        {
            ...
            {
                _smr.SetBlendShapeWeight(poseStartIndex + 0, 1000);
                _smr.SetBlendShapeWeight(poseStartIndex + 1, 1000);
                _smr.SetBlendShapeWeight(poseStartIndex + 2, 1000);

                _smr.SetBlendShapeWeight(poseStartIndex + 3, 1000);
                _smr.SetBlendShapeWeight(poseStartIndex + 4, 1000);
                _smr.SetBlendShapeWeight(poseStartIndex + 5, 1000);

                _smr.SetBlendShapeWeight(poseStartIndex + 6, 1000);
              _smr.SetBlendShapeWeight(poseStartIndex + 7, 1000);
                _smr.SetBlendShapeWeight(poseStartIndex + 8, 1000);
               print("nope"); 
            }
        }
    }
    ...
    public bool UpdateJointPositions(bool recalculateJoints = true)
    {
        if (HasBetaShapes() && recalculateJoints)
        {
          ...
          //  Matrix newJointsX = betasToJoints[0] * betaMatrix + templateJ[0];
            ...
            //Matrix newJointsX = betaMatrix+templateJ[0];;
           // Matrix newJointsY =  betaMatrix+templateJ[1];
           // Matrix newJointsZ =  betaMatrix+ templateJ[2];
            Matrix newJointsX =templateJ[0];;
            Matrix newJointsY =templateJ[1];
            Matrix newJointsZ =templateJ[2];
...
EditorGUILayout.EndHorizontal();
  ...
  EditorGUILayout.BeginHorizontal();
 EditorGUILayout.LabelField("setblandweightactive", GUILayout.Width(100));
                    if (GUILayout.Button("active"))
                    {
                       _target.issetblendshapeactive=true;
                    }
if (GUILayout.Button("inactive"))
                    {
                       _target.issetblendshapeactive=false;
                    }
                    EditorGUILayout.EndHorizontal();
                }
            }          
